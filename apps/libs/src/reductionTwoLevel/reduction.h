...
// A place's root carries both its own place's subtree and the places below it
// in the upper tree, so it needs two arities' worth of incoming channels.
#define RED_FANIN (2*ARITY)
...
typedef struct reductionPrivateBase {
    ...
    u64 nrank;
    u64 myrank;
    // Ranks per place.  The tree is built in two levels around it: a place's
    // participants reduce among themselves, and only the places' roots reduce
    // with each other.  Zero or one leaves the flat tree.
    u64 nrankPerPlace;
...
//    phase == 2, i received down
    ocrGuid_t sendUpEVT;
    ocrGuid_t recvUpEVT[RED_FANIN];
    ocrGuid_t sendDownEVT[RED_FANIN];
