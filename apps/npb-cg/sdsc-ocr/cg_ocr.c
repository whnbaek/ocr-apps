...
void conj_grad(u64 n, u64 blk, ocrGuid_t a, ocrGuid_t x, ocrGuid_t cont, u32 dep)
{
    ...
    /* The continuation stores the final zeta into this block: a write to a
       CONST acquisition is never written back, so acquire it RW. */
    ocrAddDependence(p, cont, dep+1, DB_MODE_RW);
