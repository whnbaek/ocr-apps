/* The dist decomposition's copy of the image-formation wiring.  It differs
 * from the shared source in one respect: the two tasks below bind the images
 * only to hand their GUIDs on -- neither reads a pixel -- so they take them
 * as DB_MODE_NULL.  A whole-image RO dependence on a task that forwards it
 * costs a full copy on whatever node the task lands on, and the pipeline has
 * several such hand-offs in a row. */
#include "ocr.h"
#include "rag_ocr.h"
#include "common.h"
...
#endif
RAG_DEF_MACRO_GUID_ONLY(arg_scg,curImage_dbg,0);	// Affine_edt
	}
#ifdef TRACE_LVL_2
...
RAG_DEF_MACRO_GUID_ONLY(post_FormImage_scg,curImage_dbg,2);
RAG_DEF_MACRO_GUID_ONLY(post_FormImage_scg,refImage_dbg,3);
RAG_DEF_MACRO_GUID_ONLY(post_FormImage_scg,Xin_dbg,4);
RAG_DEF_MACRO_GUID_ONLY(post_FormImage_scg,Pt_dbg,5);
RAG_DEF_MACRO_GUID_ONLY(post_FormImage_scg,Tp_dbg,6);
#ifdef RAG_DIG_SPOT
...
ocrPrintf("//// create an edt for BackProj\n");RAG_FLUSH;
        ...
        /* Which stripe set this image's gather fills. */
        backProjParamv.stripes_dbg = formImageParamvIn->stripes_dbg;
		retval = ocrEdtCreate(
...
RAG_DEF_MACRO_GUID_ONLY(BackProj_scg,curImage_dbg,2);
RAG_DEF_MACRO_GUID_ONLY(BackProj_scg,refImage_dbg,3);
/* The projection reads the pulse history only to upsample it, and it only
 * upsamples when the factor is set; otherwise it hands the block to its tiles
 * untouched, and acquiring it here would move the whole history for nothing. */
if(image_params->F > 1) {
	RAG_DEF_MACRO_PASS_RO(BackProj_scg,NULL,NULL,NULL,NULL,Xin_dbg,4);
} else {
	RAG_DEF_MACRO_GUID_ONLY(BackProj_scg,Xin_dbg,4);
}
