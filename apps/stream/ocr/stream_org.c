...
ocrGuid_t loop(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
  ...
  if (iter < ntimes) {  // Spawn another set
    ...
    /* The loop shell only forwards GUIDs and never touches payloads: RO
     * keeps it out of the arrays' write-conflict set (a writable hold would
     * overlap the kernels' RW tenures with no event ordering).  RO rather
     * than NULL because NULL-mode slots may deliver no GUID. */
    ocrAddDependence(triad_output, edt_loop, 0, DB_MODE_RO);
    ocrAddDependence(scale_output, edt_loop, 1, DB_MODE_RO);
    ocrAddDependence(add_output, edt_loop, 2, DB_MODE_RO);
  } else {
...
ocrGuid_t mainLet(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[])
{
    ...
    /* Release the init writes before wiring so the loop chain's kernels
     * acquire strictly after them (release -> satisfy -> acquire), then hand
     * the GUIDs over in RO mode — the loop shell never writes payloads
     * (see the loop-side wiring note). */
    ocrDbRelease(db_a);
    ocrDbRelease(db_b);
    ocrDbRelease(db_c);
    ocrAddDependence(db_a, edt_loop, 0, DB_MODE_RO);
    ocrAddDependence(db_b, edt_loop, 1, DB_MODE_RO);
    ocrAddDependence(db_c, edt_loop, 2, DB_MODE_RO);
